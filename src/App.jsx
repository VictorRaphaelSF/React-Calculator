...
function App() {
      ...
      if (response.ok) {
        ...
        Promise.all(
          historicoData.map((item) => {
            fetch(`http://localhost:3002/historico/${item.id}`, {
              method: 'DELETE',
              headers: {
  ...
  function saveToServer(res) {
    fetch("http://localhost:3002/historico", {
      method: 'POST',
      headers: {
         'Content-Type': 'application/json',
      },
        ...
        <div className='calc-esq'>
          <div className='div-hist'>
            ...
            <div className='label-res'>
                ...
                <p>Resultados</p>
              <button className='limpar-btn' onClick={() => btnLimparHist()}>
                <img src="../src/assets/trash-can.png"/>
              </button>
            </div>
            <div className='div-histleft'>
              {historico.map((historico) => (
                <div key={historico.id}>{historico.conta}</div>
              ))}
            </div>
          </div>
        </div>
